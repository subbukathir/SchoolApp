...
import com.daemon.oxfordschool.listeners.FeesDetailsListener;
...
{
    public static String MODULE = "GetPaymentDetail";
    public static String TAG ="";
    String Str_Msg = "",Str_Code="";
    PaymentDetailsListener mCallBack;
    String Str_Url="";
    JSONObject Payload;
    Fragment mFragment;
    ...
    public GetPaymentDetail(String Str_Url, JSONObject Payload, Fragment mFragment)
    {
        this.Str_Url = Str_Url;
        this.Payload = Payload;
        this.mFragment=mFragment;
        this.mActivity = (AppCompatActivity)mFragment.getActivity();
        mCallBack = (PaymentDetailsListener) mFragment;
        ...
        public void onResponse(Object o)
        {
            try
            {
                ...
                if (response.toString().length()==0)
                {
                    Log.d(TAG, "empty");
                    editor = mPreferences.edit();
                    editor.putString(AppUtils.SHARED_PAYMENT_DETAIL, "");
                    editor.commit();
                    Str_Msg = mActivity.getResources().getString(R.string.msg_unexpected_error);
                    mCallBack.onPaymentDetailsReceivedError(Str_Msg);
                }
                else
                {
                    ...
                    {
                        editor = mPreferences.edit();
                        editor.putString(AppUtils.SHARED_PAYMENT_DETAIL, response.toString());
                        editor.commit();
                        mCallBack.onPaymentDetailsReceived();
                    }
                    else
                    {
