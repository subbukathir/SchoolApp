...
{
    public static String MODULE = "GetFeesDetail";
    public static String TAG ="";
    String Str_Msg = "",Str_Code="";
    FeesDetailsListener mCallBack;
    String Str_Url="";
    JSONObject Payload;
    Fragment mFragment;
    ...
    public GetFeesDetail(String Str_Url, JSONObject Payload, Fragment mFragment)
    {
        this.Str_Url = Str_Url;
        this.Payload = Payload;
        this.mFragment=mFragment;
        this.mActivity = (AppCompatActivity)mFragment.getActivity();
        mCallBack = (FeesDetailsListener) mFragment;
        ...
        public void onResponse(Object o)
        {
            try
            {
                ...
                if (response.toString().length()==0)
                {
                    Log.d(TAG, "empty");
                    editor = mPreferences.edit();
                    editor.putString(AppUtils.SHARED_FEES_DETAIL, "");
                    editor.commit();
                    Str_Msg = mActivity.getResources().getString(R.string.msg_unexpected_error);
                    mCallBack.onFeesDetailsReceivedError(Str_Msg);
                }
                else
                {
                    ...
                    {
                        editor = mPreferences.edit();
                        editor.putString(AppUtils.SHARED_FEES_DETAIL, response.toString());
                        editor.commit();
                        mCallBack.onFeesDetailsReceived();
                    }
                    else
                    {
