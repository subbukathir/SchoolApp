 ...
 */
public class DisciplineReport_Process
{
    public static String MODULE = "DisciplineReport_Process";
    public static String TAG ="";

    String Str_Msg = "",Str_Code="";
    DisciplineReportListener mCallBack;
    String Str_Url=ApiConstants.DISCIPLINE_REPORT_URL;
    ...
    public DisciplineReport_Process(AppCompatActivity mActivity, Object Obj,JSONObject Payload)
    {
        this.mActivity = mActivity;
        this.Obj = Obj;
        this.Payload = Payload;
        mCallBack = (DisciplineReportListener) Obj;
        mPreferences = mActivity.getSharedPreferences(AppUtils.SHARED_PREFS, Context.MODE_PRIVATE);
        editor = mPreferences.edit();
    }

    /**
     * Getting GetDisciplineReport json by making http call
     */
        ...
        public void onResponse(Object o)
        {
            try
            {
                ...
                if (response.toString().length()==0)
                {
                    Log.d(TAG, "empty");
                    editor = mPreferences.edit();
                    editor.putString(AppUtils.SHARED_DISCIPLINE, "");
                    editor.commit();
                    Str_Msg = mActivity.getResources().getString(R.string.msg_unexpected_error);
                    mCallBack.onDisciplineReportReceivedError(Str_Msg);
                }
                else
                {
                    ...
                    {
                        editor = mPreferences.edit();
                        editor.putString(AppUtils.SHARED_DISCIPLINE, response.toString());
                        editor.commit();
                        mCallBack.onDisciplineReportReceived();
                    }
                    else
                    {
