...
{
    public static String MODULE = "GetEventsList";
    public static String TAG ="";
    String Str_Msg = "",Str_Code="";
    EventsListListener mCallBack;
    String Str_Url="";
    Fragment mFragment;
    ...
    public GetEventsList(String Str_Url, Fragment mFragment)
    {
        this.Str_Url = Str_Url;
        this.mFragment=mFragment;
        this.mActivity = (AppCompatActivity)mFragment.getActivity();
        mCallBack = (EventsListListener) mFragment;
        ...
        public void onResponse(Object o)
        {
            try
            {
                ...
                if (response.toString().length()==0)
                {
                    Log.d(TAG, "empty");
                    editor = mPreferences.edit();
                    editor.putString(AppUtils.SHARED_EVENTS_LIST, "");
                    editor.commit();
                    Str_Msg = mActivity.getResources().getString(R.string.msg_unexpected_error);
                    mCallBack.onEventsReceivedError(Str_Msg);
                }
                else
                {
                    ...
                    {
                        editor = mPreferences.edit();
                        editor.putString(AppUtils.SHARED_EVENTS_LIST, response.toString());
                        editor.commit();
                        mCallBack.onEventsReceived();
                    }
                    else
                    {
