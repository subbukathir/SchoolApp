...
{
    public static String MODULE = "GetStudentProfile";
    public static String TAG ="";
    String Str_Msg = "",Str_Code="";
    ViewStudentProfileListener mCallBack;
    String Str_Url="";
    JSONObject Payload;
    Fragment mFragment;
    ...
    public GetStudentProfile(String Str_Url, JSONObject Payload, Fragment mFragment)
    {
        this.Str_Url = Str_Url;
        this.Payload = Payload;
        this.mFragment=mFragment;
        this.mActivity = (AppCompatActivity)mFragment.getActivity();
        mCallBack = (ViewStudentProfileListener) mFragment;
        ...
        public void onResponse(Object o)
        {
            try
            {
                ...
                if (response.toString().length()==0)
                {
                    Log.d(TAG, "empty");
                    editor = mPreferences.edit();
                    editor.putString(AppUtils.SHARED_STUDENT_PROFILE, "");
                    editor.commit();
                    Str_Msg = mActivity.getResources().getString(R.string.msg_unexpected_error);
                    mCallBack.onStudentProfileReceivedError(Str_Msg);
                }
                else
                {
                    ...
                    {
                        editor = mPreferences.edit();
                        editor.putString(AppUtils.SHARED_STUDENT_PROFILE, response.toString());
                        editor.commit();
                        mCallBack.onStudentProfileReceived();
                    }
                    else
                    {
