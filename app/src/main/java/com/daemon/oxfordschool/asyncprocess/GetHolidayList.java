...
{
    public static String MODULE = "GetHolidayList";
    public static String TAG ="";
    String Str_Msg = "",Str_Code="";
    HolidayList_Listener mCallBack;
    String Str_Url="";
    JSONObject Payload;
    Fragment mFragment;
    ...
    public GetHolidayList(String Str_Url, Fragment mFragment)
    {
        this.Str_Url = Str_Url;
        this.Payload = Payload;
        this.mFragment=mFragment;
        this.mActivity = (AppCompatActivity)mFragment.getActivity();
        mCallBack = (HolidayList_Listener) mFragment;
        ...
        public void onResponse(Object o)
        {
            try
            {
                ...
                if (response.toString().length()==0)
                {
                    Log.d(TAG, "empty");
                    editor = mPreferences.edit();
                    editor.putString(AppUtils.SHARED_HOLIDAY_LIST, "");
                    editor.commit();
                    Str_Msg = mActivity.getResources().getString(R.string.msg_unexpected_error);
                    mCallBack.onHolidayListReceivedError(Str_Msg);
                }
                else
                {
                    ...
                    {
                        editor = mPreferences.edit();
                        editor.putString(AppUtils.SHARED_HOLIDAY_LIST, response.toString());
                        editor.commit();
                        mCallBack.onHolidayListReceived();
                    }
                    else
                    {
