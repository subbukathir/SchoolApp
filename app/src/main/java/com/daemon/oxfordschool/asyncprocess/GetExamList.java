...
{
    public static String MODULE = "GetExamList";
    public static String TAG ="";
    String Str_Msg = "",Str_Code="";
    ExamListListener mCallBack;
    String Str_Url="";
    JSONObject Payload;
    Fragment mFragment;
    ...
    public GetExamList(String Str_Url, JSONObject Payload, Fragment mFragment)
    {
        this.Str_Url = Str_Url;
        this.Payload = Payload;
        this.mFragment=mFragment;
        this.mActivity = (AppCompatActivity)mFragment.getActivity();
        mCallBack = (ExamListListener) mFragment;
        ...
        public void onResponse(Object o)
        {
            try
            {
                ...
                if (response.toString().length()==0)
                {
                    Log.d(TAG, "empty");
                    editor = mPreferences.edit();
                    editor.putString(AppUtils.SHARED_EXAM_LIST, "");
                    editor.commit();
                    Str_Msg = mActivity.getResources().getString(R.string.msg_unexpected_error);
                    mCallBack.onExamListReceivedError(Str_Msg);
                }
                else
                {
                    ...
                    {
                        editor = mPreferences.edit();
                        editor.putString(AppUtils.SHARED_EXAM_LIST, response.toString());
                        editor.commit();
                        mCallBack.onExamListReceived();
                    }
                    else
                    {
